...
2. 建立水平支管幾何並套用沿程管徑。
3. 以單一異徑三通直接連接水平支管與 DN25 落水段；禁止建立三通出口短管、彎頭或獨立落水變徑。灑水頭端若 Connector 並非 DN25，才依實際 Connector 管徑建立末端轉接。
...
主管接入方式修改期間，下列灑水頭末端方法以來源雜湊鎖定，避免再次破壞已成功的 Connector 連接：
...
- Crosses 規則：2 組。
- Transitions 規則：2 組。
- Junctions 規則：2 組；本專案 `FS_消防撒水管` 的 20–65 mm 範圍使用 `FS_消防撒水管_三通_螺牙`（ElementId 13155202），65–250 mm 範圍才使用焊接三通。
