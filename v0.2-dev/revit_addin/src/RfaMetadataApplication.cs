...
{
    public class RfaMetadataApplication : IExternalApplication
    {
        ...
        private static List<object> GetAvailablePipeDiameters(Document doc)
        {
            ...
            foreach (Segment segment in new FilteredElementCollector(doc).OfClass(typeof(Segment)).Cast<Segment>())
            {
                try
                {
                    foreach (MEPSize size in segment.GetSizes())
                    {
                        if (size != null && size.NominalDiameter > 0)
                        {
                            values.Add(Math.Round(UnitUtils.ConvertFromInternalUnits(size.NominalDiameter, UnitTypeId.Millimeters), 3));
                        }
                    }
                }
                catch
        ...
        private static void ProcessRequest(UIApplication uiApp, string requestFile)
        {
            ...
            try
            {
                ...
                if (action == "list_point_placement_context")
                {
                    ...
                    foreach (ImportInstance importInstance in new FilteredElementCollector(doc)
                        .OfClass(typeof(ImportInstance))
                        .Cast<ImportInstance>()
                        .OrderBy(item => GetCadImportTypeName(doc, item)))
                    {
    ...
    public static class FamilyClassifierLauncher
    {
        public static Result Open(string modeArgument, ref string message)
        {
            try
            {
                string projectRoot = @"E:\Desktop\Codex\SC REVIT";
                ...
                {
                    message = "找不到 SC REVIT 執行檔：" + exePath + "；也找不到 gui_app.py：" + guiPath;
