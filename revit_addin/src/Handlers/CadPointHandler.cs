...
namespace RfaMetadataAddin
{
    public partial class RfaMetadataApplication
    {
        ...
        private static Dictionary<string, object> SerializeCadPointProduct(
            Document doc,
            ...
            string batchId)
        {
            return new Dictionary<string, object>
            {
                ...
                { "type_name", symbol != null ? symbol.Name : "" },
                { "level_name", level != null ? level.Name : "" },
                { "block_name", blockName },
                { "location", SerializePoint(location) },
                { "x", location.X },
                { "y", location.Y },
                { "z", location.Z },
                { "batch_id", batchId }
                                ...
                                using (Transaction transaction = new Transaction(doc, "SC \u6279\u91cf\u9ede\u4f4d\u653e\u7f6e"))
                                {
                                    ...
                                    foreach (Dictionary<string, object> point in points)
                                    {
                                        try
                                        {
                                            ...
                                            created.Add(SerializeCadPointProduct(
                                                doc,
                                                instance,
                                                symbol,
                                                level,
                                                location,
                                                point["block_name"].ToString(),
                                                batchId
                                            ));
                                        }
                                        catch (Exception ex)
            ...
            if (action == "place_dwg_block_points")
                            {
                                ...
                                using (Transaction transaction = new Transaction(doc, "SC DWG \u6279\u91cf\u9ede\u4f4d\u653e\u7f6e"))
                                {
                                    ...
                                    if (createdElementIds.Count > 0)
                                    {
                                        ...
                                        foreach (Dictionary<string, object> item in created)
                                        {
